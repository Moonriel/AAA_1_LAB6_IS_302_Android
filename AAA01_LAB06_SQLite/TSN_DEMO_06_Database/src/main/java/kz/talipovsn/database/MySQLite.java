...
public class MySQLite extends SQLiteOpenHelper {
    ...
    static final String DATABASE_NAME = "NOTEPADS"; // Имя базы данных

    static final String TABLE_NAME = "notepads"; // Имя таблицы
    ...
    static final String ASSETS_FILE_NAME = "notepad.txt"; // Имя файла из ресурсов с данными для БД
    ...
    public void addData(SQLiteDatabase db, String name, String site,
                        String material, String size, String payment) {
        ...
        try {
            ...
            while ((str = in.readLine()) != null) { // Читаем строку из файла
                ...
                if (!strTrim.equals("")) { // Если строка не пустая, то
                    ...
                    addData(db, name, site, material, size, payment); // Добавляем название и телефон в базу данных
                }
            }
    ...
    public String getData(String filter, Spinner spinner) {
        ...
        if (cursor.moveToFirst()) { // Если есть хоть одна запись, то
            do { // Цикл по всем записям результата запроса
                ...
                int p = cursor.getColumnIndex(MATERIAL);
                ...
                String material = cursor.getString(p);
                String size = cursor.getString(l);
                String payment = cursor.getString(a);
                data.append(String.valueOf(++num) + ") Блокнот:" + name +
                        "\n Сайт: " + site +
                        "\n Материал: " + material +
                        "\n Формат бумаги: " + size +
